...
class ReplayBuffer():
    def __init__(self, max_size, input_shape, n_action):
        ...
        self.action_memory = zeros((self.men_size, self.n_action), dtype=int32)
...
class Agent():
    ...
    def choose_action(self, observation, argMax=False):
        if random.random() > self.epsilon:
            ...
            if argMax:
                ...
                maxA = tf.argmax(actions, axis=1).numpy()[0]
                space_actions = zeros(self.n_actions)
                space_actions[maxA] = 1
                action = space_actions

        else:
            space_actions = zeros(self.n_actions)
            action = random.choice(self.action_space)
            space_actions[action] = 1
            action = space_actions
        return action
